<div>
    <section class="mt-10">
        <div class="mx-auto px-2 lg:px-2">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Permintaan Barang</h1>
                <p class="text-gray-600 dark:text-gray-400">Kelola dan lacak semua permintaan Anda di satu tempat </p>
            </div>
            
            <flux:button variant="filled">
                <a wire:navigate href="{{ route('demand.create') }}">{{ __('+ Tambah Permintaan') }}</a> 
            </flux:button>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">nomor permintaan</th>
                                    <th scope="col" class="px-4 py-3">tanggal permintaan</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b dark:border-gray-600">
                                ...
                                <th scope="row"
                                    ...
                                    {{$item->demand_number }}</th>
                                <td class="px-4 py-3">{{ \Carbon\Carbon::parse($item->demand_date)->locale('id')->isoFormat('D MMMM Y') }}</td>
                                <td class="px-4 py-3">{{ $item->user->division->name }}</td>
                                <td class="px-4 py-3">
                                    @foreach($item->demandItems as $demandItem)
                                        {{ $demandItem->name }},
                                    @endforeach
                                </td>
                                <td class="px-4 py-3 text-green-500">
                                    <flux:badge :color="App\Models\Demand::getStatusColor($item->status)">
                                        {{ App\Models\Demand::getStatusLabel($item->status) }}
                                    </flux:badge>
                                </td>
                                <td class="px-4 py-3">
                                    ...
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute left-10 top-5 z-50 mb-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5">
                                            <div class="py-1" role="menu" aria-orientation="vertical">
                                                <button wire:click="processedDemand({{ $item->id }})" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-green-50 dark:hover:bg-green-900/20 flex items-center gap-2 cursor-pointer transition-all duration-200 ease-in-out hover:scale-[1.02] hover:shadow-sm">
                                                ...
                                                <button wire:click="$dispatch('demand.reject', { id: {{ $item->id }} })" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2 cursor-pointer transition-all duration-200 ease-in-out hover:scale-[1.02] hover:shadow-sm">
                                    ...
                                    @elseif( Auth::user()->position->id == 4 && Auth::user()->has_authority == 1 && $item->status =='processed')
                                    <div class="relative" x-data="{ open: false }">
                                        <button @click="open = !open" class="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-all duration-200 ease-in-out cursor-pointer shadow-sm hover:shadow-md">
                                            <flux:icon.clipboard-document-check class="size-5" />
                                        </button>
                                        <div x-show="open" @click.away="open = false" class="absolute left-10 top-5 z-50 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5">
                                            <div class="py-1" role="menu" aria-orientation="vertical">
                                                <button wire:click="approvedDemand({{ $item->id }})" class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-green-50 dark:hover:bg-green-900/20 flex items-center gap-2 cursor-pointer transition-all duration-200 ease-in-out hover:scale-[1.02] hover:shadow-sm">
                                                    <div class="p-1.5 bg-green-100 dark:bg-green-900/30 rounded-full">
                                                        <flux:icon.check-circle class="size-4 text-green-500" />
                                                    </div>
                                                    <span class="font-medium">Setujui</span>
                                    ...
                                    {{ 'Ditolak oleh :'}} {{ $item->rejectedDemand->position->name ?? 'Unknown' }}
                                    @elseif($item->status == 'processed')
                                    {{ 'Disetujui oleh :'}} {{ $item->processedDemand->position->name ?? 'Unknown' }}
                                    @elseif($item->status == 'approved')
                                    {{ 'Disetujui oleh :'}} {{ $item->approvedDemand->position->name ?? 'Unknown' }}
                                    @else
                                ...
                                <td class="px-4 py-3 flex items-center justify-end gap-2">
                                    ...
                                    @endif
                                    <button wire:click="$dispatch('demand.show', { id: {{ $item->id }} })" class="px-3 py-1 bg-blue-500 text-white rounded cursor-pointer">
                                        <flux:icon.eye class="size-4" />
                                    </button>
                                    <button class="px-3 py-1 bg-yellow-500 text-white rounded cursor-pointer" wire:navigate href="{{ route('demand.edit', $item->id) }}"><flux:icon.pencil-square class="size-4" /></button>
